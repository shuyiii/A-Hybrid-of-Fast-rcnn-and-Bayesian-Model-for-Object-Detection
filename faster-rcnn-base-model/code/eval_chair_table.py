...
gtpath='/fs/project/PAS1263/data/ILSVRC/test.csv'
chairlist=[];
tablelist=[];
numberchair=0;
numbertable=0;
...
MAPtable=0;MAPchair=0;
get_indexes = lambda x, xs: [i for (y, i) in zip(xs, range(len(xs))) if x <= y]
for h in range(0,11):
         x=h*0.1;
         find1=get_indexes(x,mAPtable[:,1]);
         find2=get_indexes(x,mAPchair[:,1]);
         if find1!=[]:
              MAPtable=MAPtable+1.0/11*(max(mAPtable[find1,0]));
         if find2!=[]:
              MAPchair=MAPchair+1.0/11*(max(mAPchair[find2,0]));

re=[MAPtable,MAPchair,(MAPtable+MAPchair)*1.0/2]
np.save('../outcome/chair_table_re_new',re)
